...
interface ProductionTestEnvironment {
  ...
  services: {
    auth: {
      login: jest.MockedFunction<(credentials: unknown) => Promise<unknown>>;
      logout: jest.MockedFunction<() => Promise<unknown>>;
      forgotPassword: jest.MockedFunction<(email: string) => Promise<unknown>>;
      resetPassword: jest.MockedFunction<(data: unknown) => Promise<unknown>>;
      getStoredToken: jest.MockedFunction<() => Promise<string>>;
      hasStoredToken: jest.MockedFunction<() => Promise<boolean>>;
      validateStoredSession: jest.MockedFunction<() => Promise<unknown>>;
      removeStoredToken: jest.MockedFunction<() => Promise<void>>;
    };
    payment: {
      payMyDues: jest.MockedFunction<(request: PaymentRequest) => Promise<PaymentResponse>>;
      checkStripeConfiguration: jest.MockedFunction<() => Promise<StripeConfigResponse>>;
    };
    pushNotification: {
      initialize: jest.MockedFunction<() => Promise<unknown>>;
      requestPermissions: jest.MockedFunction<() => Promise<boolean>>;
      registerPushToken: jest.MockedFunction<(token: string) => Promise<unknown>>;
